...
def entropy(class_y):
    ...
    p0 = float(totalZeros) / len(class_y)
    p1 = float(totalOnes) / len(class_y)
...
def information_gain(previous_y, current_y):
    ...
    parentEntropy = entropy(previous_y)
    childrensEntropy = 0.0
    totalParent = len(previous_y)

    for labels in current_y:
        totalChild = len(labels)
        # casting to float is slower than multiplying by 1.0. It is paython.
        childrensEntropy +=  1.0*entropy(labels)*totalChild/totalParent

    info_gain = round(1.0*parentEntropy - childrensEntropy, 5)

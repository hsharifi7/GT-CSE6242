...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        # We randomly select how many samples we need
        # This ensures that replacement does occur.  
        for i in range(n):
    ...
    def fitting(self):
        ...
        # Let's go through all trees and start the learning process for each tree
        for index, tree in enumerate(self.decision_trees):
            tree.learn(self.bootstraps_datasets[index], self.bootstraps_labels[index])
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                # We are here because all records are part of bootstraps dataset 
                votes = []
                for tree in self.decision_trees:
                    votes.append(tree.classify(record))

                y = np.append(y, np.argmax(np.bincount(votes)))
